...
def ai_by_score(mb_orig:Marubatsu, eval_func, debug=False, rand=True, analyze:bool=False) -> tuple[int, int]|list[tuple[int, int]]:
    """評価値を利用した AI のひな型.
...
def ai1s(mb:Marubatsu, debug:bool=False) -> tuple[int, int]:
...
def ai2s(mb:Marubatsu, debug:bool=False, candidate:bool=False) -> tuple[int, int]|list[tuple[int, int]]:
...
def ai3(mb:Marubatsu, analyze:bool=False) -> tuple[int, int]|dict:
    """真ん中のマスに優先的に着手する AI.
...
def ai3s(mb:Marubatsu, debug:bool=False, analyze:bool=False) -> tuple[int, int]|dict:
    ...
    Args:
        ...
        analyze:
            False の場合に、着手する座標を表す tuple を返す
            True の場合に、候補手の一覧と、それぞれの合法手に着手した場合の評価値を
            表す dictを返す。詳細は ai_by_score を参照すること            
...
def ai4s(mb:Marubatsu, debug:bool=False) -> tuple[int, int]:
...
def ai5s(mb:Marubatsu, debug:bool=False) -> tuple[int, int]:
...
def ai6s(mb:Marubatsu, debug:bool=False) -> tuple[int, int]:
...
def ai7s(mb:Marubatsu, debug:bool=False) -> tuple[int, int]:
...
def ai8s(mb:Marubatsu, debug:bool=False) -> tuple[int, int]:
...
def ai11s(mb:Marubatsu, score_201:float=2, score_102:float=0.5, score_012:float=-1, debug:bool=False) -> tuple[int, int]:
    """評価関数を利用したアルゴリズムで、以下のルールで着手を行う AI.
...
def ai12s(mb:Marubatsu, score_victory:float=300, score_sure_victory:float=200, \
...
def ai14s(mb:Marubatsu, score_victory:float=300, score_sure_victory:float=200, \
          score_defeat:float=-100, score_special:float=100, score_201:float=2, \
          score_102:float=0.5, score_012:float=-1, debug:bool=False, candidate:bool=False) -> tuple[int, int]|list[tuple[int, int]]:
    """評価関数を利用したアルゴリズムで、以下のルールで着手を行う AI.
...
def ai_gt1(mb:Marubatsu, mbtree:Mbtree) -> tuple[int, int]:
...
def ai_gt2(mb:Marubatsu, mbtree:Mbtree) -> tuple[int, int]:
    """評価が計算されたゲーム木を使って最善手を選択する強解決の AI.
    ...
    Args:
        mb: 
            現在の局面を表す Marubatsu クラスのインスタンス
        mbtree:
            ノードの bestmoves 属性が計算されている Mbtree クラスのインスタンス
...
def ai_gt3(mb:Marubatsu, mbtree:Mbtree) -> tuple[int, int]:
    """評価が計算されたゲーム木を使って最善手を選択する強解決の AI.
    ...
    Args:
        mb: 
            現在の局面を表す Marubatsu クラスのインスタンス
        mbtree:
            ノードの bestmoves 属性 と nodelist_by_mb 属性が
            計算されている Mbtree クラスのインスタンス
...
def ai_gt4(mb:Marubatsu, mbtree:Mbtree)-> tuple[int, int]:
    """評価が計算されたゲーム木を使って最善手を選択する強解決の AI.
    ...
    Args:
        mb: 
            現在の局面を表す Marubatsu クラスのインスタンス
        mbtree:
            bestmoves_by_mb 属性が計算されている Mbtree クラスのインスタンス
...
def ai_gt5(mb:Marubatsu, bestmoves:dict[tuple, list[tuple[int, int]]]) -> tuple[int, int]:
    """評価が計算されたゲーム木を使って最善手を選択する強解決の AI.
...
def ai_gt6(mb, bestmoves_by_board:dict[str, list[tuple[int, int]]], candidate=False) -> tuple[int, int]:
    """評価が計算されたゲーム木を使って最善手を選択する強解決の AI.
