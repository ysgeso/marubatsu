...
def calc_pdist(pdistA:dict, pdistB:dict) ->dict:
    """深さの上限を 1 とする近似値のミニマックス値の確率分布を計算する.
    
    近似値の確率分布が pdistA と pdistB の子ノードを持つ max ノードの
    近似値のミニマックス値の確率分布を計算して返す
    
    Attributes:
        pdistA, pdistB:
            2 つの子ノードに対して計算される近似値の確率分布を表す dict
            キーを確率変数とし、キーの値に確率が代入されているものとする
            
    Returns:
        max ノードの近似値のミニマックス値の確率分布を表す dict
    """
