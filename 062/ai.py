...
def ai12s(mb:Marubatsu, score_201:float=2, score_102:float=0.5, score_012:float=-1, debug:bool=False) -> tuple[int, int]:
    """評価関数を利用したアルゴリズムで、以下のルールで着手を行う AI.

    以下のルールで着手を行う
    1. 真ん中のマスに優先的に着手する（評価値 400）
    2. そうでない場合は、勝てる場合に勝つ（評価値 300）
    3. そうでない場合は、相手が勝利できる着手を行わない（評価値 -100）
    4. そうでない場合は、次 の 自分の手番で必ず勝利できるように、
       「自 2 敵 0 空 1」が 2 つ以上存在する着手を行う（評価値 200）
    5. そうでない場合は、斜め方向に 〇×〇 が 並び、他の 6 マスが
       空のマスの場合は、いずれかの辺のマスに着手を行う（評価値 100）       
    6. そうでない場合は、以下の条件の組み合わせで着手を行う（評価値 は下記の合計）
      6.1. 次の自分の手番で勝利できるように、
          「自 2 敵 0 空 1」が 1 つ存在する局面になる着手を行う（評価値 score_201）
      6.2. 次の自分の手番で有利になるように、
          「自 1 敵 0 空 2」が最も多くなる局面になる着手を行う（評価値 1 つあたり score_102）
      6.3. 次の自分の手番で有利になるように、
          「自 0 敵 1 空 2」が最も少なくなる局面になる着手を行う（評価値 1 つあたり score_012）
    7. そうでない場合はランダムなマスに着手する    
