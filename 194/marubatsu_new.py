...
class ListBoard(Board):
    ...
    def is_same(self, mark:str, coord:tuple[int, int], dx:int, dy:int) -> bool:
    ...
    def count_marks(self, turn:str, last_turn:str, coord:list[int], dx:int, dy:int, datatype:str="dict") -> defaultdict | tuple[int, int, int]:
...
class Marubatsu:
    ...
    def move(self, x: int, y: int, check_coord:bool=False):
        """ ゲーム盤の指定したマスに着手する.
        ...
        Args:
            ...
            y:
                マークを配置するマスの y 座標
            check_coord:
                True の場合に座標のチェックを行う place_mark で着手する
                Falst の場合は座標のチェックを行わない board.setmark で着手する
        """
