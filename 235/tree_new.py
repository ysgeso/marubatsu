...
class Mbtree:
    ...
    def calc_score_of_node(self, node:Node):
        ...
        if node.mb.status == node.mb.CIRCLE:
            node.score = (11 - node.depth) / 2 if self.shortest_victory else 1
        elif node.mb.status == node.mb.DRAW:
            node.score = 0
        elif node.mb.status == node.mb.CROSS:
    ...
    def calc_score_by_ab(self, abroot:Node, debug:bool=False, minimax:bool=False, init_ab:bool=False, use_tt:bool=False):
        ...
        def calc_ab_score(node, tt, alpha=float("-inf"), beta=float("inf")):
            ...
            if use_tt:
                ...
                if key in tt:
                    node.score = tt[key]
                    if node.mb.turn == node.mb.CIRCLE:
                        alpha = node.score
                    else:
                        beta = node.score
                    self.nodelist_by_score.append(node)
        ...
        def calc_node_score(node, tt, alphaorig=float("-inf"), betaorig=float("inf")):
            ...
            if node.mb.status != node.mb.PLAYING:        
                ...
                if not tt_pruned:
                    ...
                    beta = betaorig
                    ab_pruned = False
                    maxnode = node.mb.turn == node.mb.CIRCLE
    ...
    def calc_playout_prob_by_df(self, node):
        """このノードの局面からプレイアウトを行った場合の結果の確率を計算する."""
    ...
    def sort_children_by_worst_score(self):
        ...
        for node in self.nodelist:
            reverse = False if node.mb.turn == node.mb.CIRCLE else True
...
class Mbtree_Anim(GUI):
    ...
    def update_ab(self):
        ...
        maxnode = self.selectednode.mb.turn == self.selectednode.mb.CIRCLE
...
def calc_discrete_ndist(m:int, s:float, area:int) -> dict:
    """整数の確率変数を持つ正規表現に従う離散型確率を計算する.
    ...
    Attributes:
        ...
        area:
            計算する確率変数の範囲を表す整数
            m - area 以上 m + area 以下の確率変数に対する確率を計算する
...
def translate_pdist(pdist:dict, diff:float) -> dict:
    """確率分布の確率変数をずらした確率分布を計算する.
...
def calc_pdistlist(maxdepth:int, std:float, area:int) ->list[dict]:
    """指定した深さの上限までのルートノードの確率分布を表す list を計算する.
    
    Attributes:
        maxdepth:
            深さの上限を表す整数
        std:
            静的評価関数の確率分布の標準偏差を表す数値
        area:
            静的評価関数の確率分布のの確率変数の範囲を表す整数
            - area 以上 + area 以下の確率変数に対する確率を計算する
            
    Returns:
        インデックスを深さの上限とし、深さの上限の確率分布を要素とする list
    """       
...
def draw_pdistlist(pdistlist:list[dict]):
    """確率分布の一覧の平均と標準偏差をグラフで表示する.
