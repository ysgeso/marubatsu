...
def ai_by_score(eval_func):
    ...
    def wrapper(mb_orig:Marubatsu, debug:bool=False, *args, rand:bool=True, 
        ...
        if calc_score:
            ...
            if minimax and mb_orig.turn == mb_orig.CIRCLE:
                score *= -1
            return score
...
def ai_by_mmscore(eval_func):
    ...
    def wrapper(mb_orig, debug=False, *args, rand=True, share_tt=True,
        ...
        legal_moves = mb_orig.calc_legal_moves()
        dprint(debug, "legal_moves", legal_moves)
        maxnode = mb_orig.turn == mb_orig.CIRCLE
        ...
        if analyze:
            if share_tt:
                ...
                while mb.status == mb.PLAYING:
                    PV.append(bestmove)
                    if mb.board.getmark_by_move(bestmove) != mb.EMPTY:
                        print("そのマスには着手済みです")
                        break
                    mb.move(bestmove)
...
def ai3(mb:Marubatsu, debug:bool=False) -> list[tuple[int, int]]:
    ...
    if mb.board.getmark(1, 1) == mb.EMPTY:
...
def ai6(mb_orig:Marubatsu, debug:bool=False) -> list[tuple[int, int]]:
    ...
    for move in legal_moves:
        mb = deepcopy(mb_orig)
        # 現在の局面の手番を入れ替える
        mb.turn = mb.CROSS if mb.turn == mb.CIRCLE else mb.CIRCLE
        enemy_turn = mb.turn
        mb.move(move)
...
def ai12s(mb:Marubatsu, debug:bool=False, score_victory:float=300, score_sure_victory:float=200, \
    ...
    if mb.board.getmark(1, 1) == mb.CROSS and \
        (mb.board.getmark(0, 0) == mb.board.getmark(2, 2) == mb.CIRCLE or \
        mb.board.getmark(2, 0) == mb.board.getmark(0, 2) == mb.CIRCLE) and \
        (mb.board.getmark(1, 0) == mb.CROSS or \
        mb.board.getmark(0, 1) == mb.CROSS or \
        mb.board.getmark(2, 1) == mb.CROSS or \
        mb.board.getmark(1, 2) == mb.CROSS) and \
        mb.move_count == 4:
...
def ai14s(mb:Marubatsu, debug:bool=False, score_victory:float=300, score_sure_victory:float=200, \
    ...
    if  mb.move_count == 4 and \
        mb.board.getmark(1, 1) == mb.CROSS and \
        (mb.board.getmark(0, 0) == mb.board.getmark(2, 2) == mb.CIRCLE or \
        mb.board.getmark(2, 0) == mb.board.getmark(0, 2) == mb.CIRCLE) and \
        (mb.board.getmark(1, 0) == mb.CROSS or \
        mb.board.getmark(0, 1) == mb.CROSS or \
        mb.board.getmark(2, 1) == mb.CROSS or \
        mb.board.getmark(1, 2) == mb.CROSS):
        return score_special    
...
def ai_mmdfs(mb:Marubatsu, debug:bool=False) -> float:
    ...
    def mm_search(mborig):
        nonlocal count
        count += 1
        if mborig.status == mborig.CIRCLE:
            return 1
        elif mborig.status == mborig.CROSS:
            return -1
        elif mborig.status == mborig.DRAW:
            return 0
...
def ai_abs_dls(mb:Marubatsu, debug:bool=False, timelimit_pc:float|None=None, maxdepth:int=1,
    ...
    def ab_search(mborig, depth, tt, alpha=float("-inf"), beta=float("inf")):
        ...
        count += 1
        if mborig.status != mborig.PLAYING or depth == maxdepth:
